...
export const STATE_KEYS = {
    ...
    LAST_SUCCESS_STREAK: 'faultline.lastSuccessStreak',
    BOSS_HP: 'faultline.bossHp',
...
export class StateStore {
    ...
    public getLastSuccessStreak(): number {
        return this.globalState.get<number>(STATE_KEYS.LAST_SUCCESS_STREAK, 0);
    }

    public async updateLastSuccessStreak(streak: number): Promise<void> {
        await this.globalState.update(STATE_KEYS.LAST_SUCCESS_STREAK, streak);
    }

    public getBossHp(): number {
        return this.globalState.get<number>(STATE_KEYS.BOSS_HP, 100);
    }

    public async updateBossHp(hp: number): Promise<void> {
        await this.globalState.update(STATE_KEYS.BOSS_HP, hp);
    }
